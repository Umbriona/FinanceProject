...
def parse_open(file_path='E:\\Financial_data\\open\\Amd_open.txt'):
    ...
    data_tensor = np.zeros([20, len(split_n_list)-1])
    j = 0
    for i in split_n_list:
        ...
        j += 1
    m = 0
    while np.all(data_tensor[length_data - m, :] == 0):
        data_tensor = np.delete(data_tensor, length_data - m, axis=0)
        m += 1
...
def parse_closed(file_path='E:\\Financial_data\\closed\\Amd_closed.txt'):
    ...
    data_tensor = np.zeros([17, len(split_n_list)-1])
    j = 0
    for i in split_n_list:
        temp = i.split(' ')
        while '' in temp:
    ...
    m = 0
    while np.all(data_tensor[length_data - m, :] == 0):
        np.delete(data_tensor, length_data - m, axis=0)
    m += 1
    return data_tensor
